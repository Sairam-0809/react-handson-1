  ...
  return (
    <>
      <div className='parentContainer'>
        <div>
          <h2 className='news-head'>Food Latest</h2>
          {data.filter((data)=> data.footer === "Food Latest").map((item ,index)=>{
            // console.log(item);
            console.log(item.id)
            return (
              <div key={index}>
                  <Link to={`/detailsfood/${item.id}`}>
          ...
          <h2 className='news-head'>Food Top</h2>
          {data.filter((data)=> data.footer === "Food Top").map((item ,index)=>{
            // console.log(item);
            console.log(item.id)
            return (
              <div key={index}>
