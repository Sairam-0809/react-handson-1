    ...
    return (
        <>        
            ...
            <div className='back-button'>
                <button className='back' onClick={()=>Navi('/food')}>BACK</button>
                <button className='back-home' onClick={()=>Navi("/")}>BACK HOME</button>
            </div>
                <h2>Food Top</h2>
                <div className='top-parent'>
                {contextData.filter((contextData)=> contextData.footer === "Food Top").map((item ,index)=>{
                    // console.log(item);
                    console.log(item.id)
                    return (
                    <div className='bolly-top' key={index}>
                        <Link className='child' to={`/detailsfood/${item.id}`}>                     
...
export default DynamicCompoFood;